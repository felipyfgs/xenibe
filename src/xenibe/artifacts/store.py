...
def write_compact_run_artifacts(run_dir: Path, run_doc: dict[str, Any], records: list[dict[str, Any]], report: str) -> None:
    ...
    _write_legacy_run_projection(run_dir, normalized, records, report)


def _write_legacy_jsonl_projection(path: Path, records: list[dict[str, Any]]) -> None:
    path.write_text("", encoding="utf-8")
    for record in records:
        append_jsonl(path, record)


def _write_legacy_run_projection(run_dir: Path, run_doc: dict[str, Any], records: list[dict[str, Any]], report: str) -> None:
    run_id = str(run_doc.get("runId") or run_dir.name)
    manifest = _manifest_from_compact(run_doc)
    manifest.setdefault("runId", run_id)
    manifest.setdefault("status", run_doc.get("status", "completed"))
    write_json(run_dir / "manifest.json", manifest)

    config_snapshot = run_doc.get("configSnapshot")
    write_yaml(run_dir / "config-snapshot.yml", config_snapshot if isinstance(config_snapshot, dict) else {})

    inputs = dict(run_doc.get("inputs")) if isinstance(run_doc.get("inputs"), dict) else {}
    inputs.setdefault("runId", run_id)
    if run_doc.get("subject") is not None:
        inputs.setdefault("subject", run_doc["subject"])
    write_json(run_dir / "inputs.json", inputs)

    scoreboard = dict(run_doc.get("scoreboard")) if isinstance(run_doc.get("scoreboard"), dict) else {}
    scoreboard.setdefault("runId", run_id)
    scoreboard.setdefault("rankings", {"candidates": []})
    scoreboard.setdefault("components", [])
    write_json(run_dir / "scoreboard.json", scoreboard)

    metrics = run_doc.get("metrics") if isinstance(run_doc.get("metrics"), dict) else {}
    write_json(run_dir / "metrics.json", {"runId": run_id, "status": run_doc.get("status", "completed"), "metrics": metrics})
    ...
    kind_to_filename = {
        ...
        "block": "blocks.jsonl",
        "equity": "equity.jsonl",
        "horizon": "horizons.jsonl",
    }
    for record in records:
        kind = record.get("kind")
        data = record.get("data")
        filename = kind_to_filename.get(kind)
        if filename is not None and isinstance(data, dict):
            records_by_filename[filename].append(data)
    for filename, legacy_records in records_by_filename.items():
        _write_legacy_jsonl_projection(run_dir / filename, legacy_records)
    (run_dir / "report.md").write_text(report, encoding="utf-8")
