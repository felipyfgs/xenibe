...
def dispatch(args: list[str], context: CommandContext) -> dict[str, Any]:
    ...
    if not timeframe or not start or not end:
        return fail("missing-name", "history download requires --timeframe, --from, and --to", ["forge history download EURUSD --timeframe M1 --from 2026-01-01 --to 2026-01-02 --json"])
    result = service.download(context, args[1], timeframe, start, end)
    if "error" in result:
