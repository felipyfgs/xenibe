...
def dispatch(args: list[str], context: CommandContext) -> dict[str, Any]:
    ...
    if command in {"generate", "show"}:
        ...
        status = dry_status(context.dry_run, "ok") if command == "generate" else "ok"
        return ok(result, [f"forge compare runs {args[1]} {args[2]} <other-run-id> --root {context.root} --json", f"forge promote run {args[1]} {args[2]} --root {context.root} --json"], status)

    if len(args) >= 2:
        result = service.show(context.root, args[0], args[1])
        error = result_error(result)
        if error is not None:
            return fail(error[0], error[1], [f"forge run validate {args[0]} {args[1]} --root {context.root} --json"])
