...
from xenibe.artifacts.store import ValidationIssue, experiment_dir, list_experiment_dirs, validate_config, validate_experiment_dir, validate_promoted_catalog, validate_run_dir
...
def validate_root(root: Path) -> dict[str, Any]:
    ...
    issues.extend(validate_promoted_catalog(root))
    for experiment in list_experiment_dirs(root):
        ...
        if runs.exists():
            for run_dir in runs.iterdir():
                if run_dir.is_dir():
                    issues.extend(validate_run_dir(run_dir, expected_experiment=experiment))
    if issues:
        return {"artifactRoot": str(root), "valid": False, "issues": issues_payload(issues)}
